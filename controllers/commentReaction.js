...
const setCommentReaction = async (liked, disliked, commentId, userId) => {
  const oldCommentReaction = await CommentReaction.findOne({
    where: { commentId, commentLikedBy: userId }
  });
...
export default {
  async likeComment(req, res) {
    await setCommentReaction(true, false, req.params.id, req.decoded.id);
    return res.sendStatus(200);
  },

  async dislikeComment(req, res) {
    await setCommentReaction(false, true, req.params.id, req.decoded.id);
    return res.sendStatus(200);
  },
};
