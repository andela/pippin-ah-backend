...
describe('ARTICLE TEST SUITE', () => {
  ...
  describe('Get Article by category', () => {
    it('should not get article when the category does not match', async () => {
      const response = await chai.request(server)
    ...
    it('should return empty array when category is undefined', async () => {
      const response = await chai.request(server)
    ...
    it('should get article when the category matches', async () => {
      const response = await chai.request(server)
        .get('/api/v1/articles/categories?category=Science')
        .set('Authorization', accesstoken);

      expect(response.status).to.equal(200);
      expect(response.body[0].author).to.equal('davidhook');
